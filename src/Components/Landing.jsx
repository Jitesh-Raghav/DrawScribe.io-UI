...
const Landing = () => {
  ...
  const handleStartClick = () => {
    if (userInput.toLowerCase() === 'i am ready') {
      setReady(true);
      startAudio(); // Start the audio after user confirms readiness
    } else {
      alert("Please enter 'I am ready' to proceed.");
    }
  };
  ...
  return (
    <>
    {!ready?( // Form asking the user to type "I am ready"
    <div className='bg-black h-screen w-full flex items-center justify-center'>
      <div className="text-center ">
        <h1 className="text-white text-4xl mb-4 font-Mario">Enter "I am ready" to start:</h1>
        <input
          type="text"
          value={userInput}
          onChange={handleInputChange}
          placeholder="Type 'I am ready'"
          className="px-4 py-2 text-sm rounded placeholder:font-Mario"
        />
        <div>
          <button
            onClick={handleStartClick}
            className="mt-4 px-6 py-2 text-white bg-green-500 rounded-lg font-Mario"
          >
            Enter
          </button>
        </div>
      ...
      >
        ...
        >
          ...
          {/* Optional message for the user to click to enable audio */}
          {/* {!audioRequested && (
            <p className="mt-4 text-white text-lg">Click to enable sound and start the game!</p>
          )} */}
        </div>
      </div>)};
      </>
